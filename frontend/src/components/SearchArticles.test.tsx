...
describe("SearchArticles Component", () => {
  ...
  test("renders search form correctly", () => {
    ...
    expect(
      screen.getByPlaceholderText(
        "Enter keywords to search across titles, authors, and claims...",
      ),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByLabelText("Select SE Practice type"),
    ).toBeInTheDocument();
  ...
  test("handles evidence type filter", async () => {
    ...
    fireEvent.change(evidenceTypeSelect, {
      target: { value: EvidenceType.MODERATELY_SUPPORTS },
    });
  ...
  test("handles multiple filters", async () => {
    ...
    const evidenceTypeSelect = screen.getByLabelText("Select SE Practice type");
    fireEvent.change(evidenceTypeSelect, {
      target: { value: EvidenceType.WEAK_AGAINST },
    });
    ...
    await waitFor(() => {
      const fetchCallUrl = (global.fetch as jest.MockedFunction<typeof fetch>)
        .mock.calls[0][0];
